...
void bsp_touch_read(void)
{
    ...
    g_touch_data.touch_num = data[1] & 0x0F;
...
bool bsp_touch_read_raw(uint8_t *data, uint8_t length)
{
    if (data == NULL || length == 0)
        return false;
    return touch_i2c_read(AXS5106L_ADDR, AXS5106L_TOUCH_DATA_REG, data, length);
}
